...
def generate_multihorizon_targets(df):
    """
    Takes the base vegetable dataframe (output of Vegetable_feature_v1.py) and generates
    ...
    for horizon in range(1, 31):
        ...
        # Stationary percentage-change target
        # +1e-6 in denominator guards against division by zero
        pct_col = f'target_{horizon}d_pct_change'
        df[pct_col] = (df[lead_col] - df['modal_price']) / (df['modal_price'] + 1e-6)

    # Note: We DO NOT drop NAs here, or else we lose the most recent 30 days
    # of feature sets needed by `predict.py` for true future inference.
...
if __name__ == "__main__":
    ...
    if not os.path.exists(v1_path):
        raise FileNotFoundError(f"Missing {v1_path}. Run Vegetable_feature_v1.py first.")

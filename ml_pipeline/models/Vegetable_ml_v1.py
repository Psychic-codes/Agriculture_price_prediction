...
print("  Reads     : ml_pipeline/data/Vegetable_Feature_v2_Multi.csv")
print("=" * 72)

df = pd.read_csv('ml_pipeline/data/Vegetable_Feature_v2_Multi.csv', parse_dates=['date'])
...
df_global = pd.get_dummies(df, columns=['commodity','Month'], dtype=int)
X_cols = [f for f in NUMERIC_FEATURES if f in df_global.columns] + \
         [c for c in df_global.columns if c.startswith('commodity_')]

# Sort chronologically for proper time-series validation
X_global = df_global[X_cols].copy()

# ── Drop non-numeric columns that crept into X_cols (e.g. 'Month', 'State') ──
non_numeric = X_global.select_dtypes(exclude=[np.number]).columns.tolist()
if non_numeric:
    print(f"  [WARN] Dropping non-numeric columns from X_cols: {non_numeric}")
    X_global = X_global.drop(columns=non_numeric)
...
NUMERIC_FEATURES = [f for f in NUMERIC_FEATURES if f not in ('Month', 'State')]
...
# ── Phase 5 Metric Optimisation: Block Extrapolation of Black-Swan Anomalies ──
# Prevent models from learning variance beyond 150% spikes and 80% market crashes
y_train = np.clip(y_train, -0.80, 1.50)

...
def get_forecasts_for_pipeline(feature_csv, model_pkl):
    ...
    if 'XGBoost' in base_estimators:
        try:
            explainer = shap.TreeExplainer(base_estimators['XGBoost'].estimators_[0])
            shap_vals_matrix = explainer.shap_values(X_latest)
            print(f"[INFO] Analyzed native AI logic paths natively utilizing SHAP TreeExplainer.")
    ...
    for i, (_, row) in enumerate(latest_rows.iterrows()):
        ...
        if shap_vals_matrix is not None:
             comm_shap = shap_vals_matrix[i:i+1] # Array slice retaining 2D form
             ...
             shap.summary_plot(comm_shap, pd.DataFrame(X_latest[i:i+1], columns=expected_features), plot_type="bar", max_display=10, show=False, color="#2E7D32")
             plt.title(f"Live AI Drivers ({commodity.title()})", fontsize=15, pad=20, fontweight='bold', color='#1a1a1a')
             plt.xlabel("Average Impact on Price Trajectory Output", fontsize=11, fontweight='bold', color='#454745', labelpad=10)
             plt.tight_layout()
             
             clean_name = commodity.replace(' ', '_').replace('(', '').replace(')', '')
             plt.savefig(f"plots/{clean_name}_shap.png", dpi=250, bbox_inches='tight', transparent=True)
             plt.close()
...
def generate_all_predictions():
    cereal_forecasts = get_forecasts_for_pipeline('ml_pipeline/data/Cereal_Feature_v2_Multi.csv', 'ml_pipeline/models/saved_models/global_stacking_30d.pkl')
    veg_forecasts = get_forecasts_for_pipeline('ml_pipeline/data/Vegetable_Feature_v2_Multi.csv', 'ml_pipeline/models/saved_models/global_veg_stacking_30d.pkl')
    ...
    print(f"[SUCCESS] Multi-Horizon Forecasts with Confidence Intervals saved to {out_file}")
